...
def df(drive_mnt):
    ...
    try:
        ...
        drive_status = (
            "Drive [ %s ] Mount_Point [ %s ] Space_Used [ %s %s of %s ] Space_Avail [ %s ]"
...
class DirectoryHandler(SimpleHTTPRequestHandler):
    def list_directory(self, path):
        ...
        if web_list_by_datetime:
            ...
            list.sort(
                key=lambda x: os.stat(os.path.join(path, x)).st_mtime,
        ...
        if web_page_refresh_on:
            f.write(
                b'<meta http-equiv="refresh" content="%s" />'
        ...
        list_style = (
            b'<div style="height: '
            + web_list_height.encode("utf-8")
            + b'px; overflow: auto; white-space: nowrap;">'
        )
        f.write(list_style)
...
VALUE="Refresh">&nbsp;&nbsp;<b>%s</b></FORM>"""
        ...
        if self.path != "/":  # Display folder Back arrow navigation if not in web root
            f.write(
                b'<li><a href="%s" >%s</a></li>\n'
                % (
                    urllib.parse.quote("..").encode("utf-8"),
                    html.escape("< BACK").encode("utf-8"),
                )
            )
        display_entries = 0
        file_found = False
        for name in list:
            ...
            if os.path.isdir(fullname):  # check if entry is a directory
                ...
                f.write(
                    b'<li><a href="%s" target="imgbox">%s</a> - %s</li>\n'
                    % (
                        urllib.parse.quote(linkname).encode("utf-8"),
                        html.escape(displayname).encode("utf-8"),
                        date_modified.encode("utf-8"),
                    )
                )
        ...
        f.write(
            b'<div style="float: left; padding-left: 40px;">Web Root is [ %s ]  %s</div>'
            % (web_server_root.encode("utf-8"), drive_status.encode("utf-8"))
        )
        f.write(
            b'<div style="text-align: center;">%s</div>'
            % web_page_title.encode("utf-8")
        )

        if web_page_refresh_on:
            f.write(
                b'<div style="float: left; padding-left: 40px;">Auto Refresh = %s sec</div>'
                % web_page_refresh_sec.encode("utf-8")
            )

        if web_max_list_entries > 1:
            f.write(
                b'<div style="text-align: right; padding-right: 40px;">Listing Only %i of %i Files in %s</div>'
                % (
                    display_entries.encode("utf-8"),
                    all_entries.encode("utf-8"),
                    self.path.encode("utf-8"),
                )
            )
        else:
            f.write(
                b'<div style="text-align: right; padding-right: 50px;">Listing All %i Files in %s</div>'
        ...
        self.send_header(
            b"Content-type", b"text/html; charset=%s" % encoding.encode("utf-8")
        )
...
net_interface_names = [
    b"eth0",
    b"wlan0",
]  # byte string list of interface names to check
ip_list = []
...
print(
    "          web_iframe_width = %s  web_iframe_height = %s"
    % (web_iframe_width, web_iframe_height)
)
print(
    "          web_iframe_width_usage = %s (of avail screen)" % (web_iframe_width_usage)
)
print("          web_page_refresh_sec = %s  (default=180 sec)" % web_page_refresh_sec)
print(
    "          web_page_blank = %s ( True=blank left pane until item selected)"
    % web_page_blank
)
print("Listing - web_max_list_entries = %s ( 0=all )" % web_max_list_entries)
print(
    "          web_list_by_datetime = %s  sort_decending = %s"
    % (web_list_by_datetime, web_list_sort_descending)
)
print("----------------------------------------------------------------")
