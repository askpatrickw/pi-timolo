...
HORIZ_LINE = "-------------------------------------------------------"
print(HORIZ_LINE)
print(f"{PROG_NAME} written by Claude Pageau")
print(HORIZ_LINE)
print("Loading Wait ....")
...
## TODO: Refactor Pantilt support so it doesn't download files and uses public
...
if PANTILT_ON:
    ...
    if PANTILT_IS_PIMORONI:
        ...
        try:
            ...
            print(
                "       if pimoroni pantilt installed check that I2C enabled in raspi-config."
            )
...
for x in range(0, 2):
    if params[x].find("0") >= 0:
        ...
        logging.error(
            "  if detected=0 Check Pi Camera Module and cable is Installed Correctly."
        )
        logging.error("Pi-Timolo Exiting Due to Error")
        sys.exit(1)
else:
...
if PLUGIN_ON:  # Check and verify plugin and load variable overlay
    ...
    if not os.path.isdir(pluginDir):
        ...
        logging.error("Rerun ./install-pi-timolo.sh to create plugin Directory")
        logging.error("Exiting %s Due to Error", PROG_NAME)
        sys.exit(1)
...
def checkConfig():
    ...
    if (
        not MOTION_TRACK_ON
        and not TIMELAPSE_ON
        and not PANTILT_SEQ_ON
        and not PANO_ON
        and not VIDEO_REPEAT_ON
    ):
        errorText = (
...
def takePantiltSequence(filename, daymode, pix_ave, num_count, num_path):
    ...
    if (not daymode) and PANTILT_SEQ_DAYONLY_ON:
        logging.info(
            "Skip since PANTILT_SEQ_DAYONLY_ON = %s and daymode = %s",
    ...
    if MOTION_TRACK_ON and MOTION_TRACK_PANTILT_SEQ_ON:
        ...
        if PANTILT_SEQ_ON:
            logging.warning(
                "MOTION_TRACK_PANTILT_SEQ_ON takes precedence over PANTILT_SEQ_ON"
            )
...
def takePano(pano_seq_num, daymode, pix_ave):
    ...
    if (not daymode) and PANO_DAYONLY_ON:
        logging.info(
            "Skip since PANO_DAYONLY_ON = %s and daymode = %s", PANO_DAYONLY_ON, daymode
        )
        return
...
def timolo():
    ...
    while True:  # Start main program Loop.
        ...
        if (
            ...
            logging.warning("Exiting pi-timolo \n")
            sys.exit(1)
...
if __name__ == "__main__":
    ...
    try:
        ...
        if VERBOSE_ON:
            ...
            logging.info(f"Exiting {PROG_NAME}")
        else:
            sys.stdout.write("User Pressed Keyboard ctrl-c \n")
            sys.stdout.write(f"Exiting {PROG_NAME}\n")
    try:
        if PLUGIN_ON:
            if os.path.isfile(pluginCurrent):
                os.remove(pluginCurrent)
