...
PLUGIN_NAME = (
    "shopcam"  # Specify filename in plugins subfolder without .py extension per below
)
...
LOG_TO_FILE_ON = (
    False  # Default= False True logs diagnostic data to a disk file for review
)
DEBUG_ON = (
    False  # Default= False True= DEBUG_ON mode returns pixel average data for tuning
)
...
IMAGE_PREVIEW = (
    False  # Default= False True=Preview image on connected RPI Monitor or Display
)
IMAGE_PIX_AVE_TIMER_SEC = 15  # Default= 15 Interval seconds for checking pixAverage Readings (reduces cpu usage)
IMAGE_NO_NIGHT_SHOTS = (
    False  # Default= False True=No Night Images (Motion or Timelapse)
)
...
STREAM_FPS = (
    20  # Default= 20 fps PiVideoStream setting.  Single core RPI suggest 15 fps
)
STREAM_STOP_SEC = (
    0.7  # Default= 0.7 Allow time to stop video stream thread to release camera
)
...
NIGHT_TWILIGHT_THRESHOLD = (
    90  # Default= 90 dayPixAve where twilight starts (framerate_range shutter)
)
NIGHT_DARK_THRESHOLD = (
    50  # Default= 50 dayPixAve where camera variable shutter long exposure starts
)
NIGHT_BLACK_THRESHOLD = (
    4  # Default= 4  dayPixAve where almost no light so Max settings used
)
NIGHT_SLEEP_SEC = (
    20  # Default= 20 Sec - Time period to allow camera to calculate low light AWB
)
NIGHT_MAX_SHUT_SEC = (
    5.9  # Default= 5.9 Sec Highest V1 Cam shutter for long exposures V2=10 Sec.
)
NIGHT_MAX_ISO = (
    800  # Default= 800 Night ISO setting for Long Exposure below nightThreshold
)
...
TIMELAPSE_PREFIX = (
    "tl-"  # Default= "tl-" Prefix for All timelapse images with this prefix
)
...
TIMELAPSE_CAM_SLEEP_SEC = (
    4.0  # Default= 4.0 seconds day sleep so camera can measure AWB before taking photo
)
TIMELAPSE_NUM_ON = (
...
TIMELAPSE_NUM_MAX = (
    2000  # Default= 2000 Max number of timelapse images desired. 0=Continuous
)
TIMELAPSE_EXIT_SEC = (
...
MOTION_TRACK_INFO_ON = (
    True  # Default= True False= Hide detailed track progress logging messages
)
MOTION_TRACK_TIMEOUT_SEC = (
    0.3  # Default= 0.3 seconds Resets Track if no movement tracked
)
MOTION_TRACK_TRIG_LEN = (
    50  # Default= 75 px Length of motion track to Trigger motionFound
)
MOTION_TRACK_MIN_AREA = (
    100  # Default= 100 sq px  Minimum Area required to start tracking
)
...
MOTION_RECENT_MAX = (
    40  # Default= 40 0=Off or specify number of recent files in MOTION_RECENT_DIR
)
MOTION_START_AT = ""  # Default= "" Off or Specify date/time to Start Sequence Eg "01-jan-20018 08:00:00" or "20:00:00"
MOTION_NUM_ON = (
    True  # Default= True filenames by sequenced Number  False= filenames by date/time
)
MOTION_NUM_RECYCLE_ON = (
    True  # Default= True when NumMax reached restart at NumStart instead of exiting
)
...
MOTION_CAM_SLEEP = (
    0.7  # Default= 0.7 Sec of day sleep so camera can measure AWB before taking photo
)
...
MOTION_TRACK_QUICK_PIC_BIGGER = (
    3.0  # Default= 3.0 multiply size of QuickPic saved image from Default 640x480
)
...
MOTION_TRACK_MINI_TL_TIMER_SEC = (
    5  # Default= 5 secs between each image. 0 is as fast as possible
)

MOTION_TRACK_PANTILT_SEQ_ON = (
    False  # Default= False True= Takes Pantilt Images per PANTILT_SEQ_STOPS below.
)
...
MOTION_VIDEO_TIMER_SEC = (
    10  # Default= 10 secs Duration of single Video clip to take after Motion Detected
)
# ---------------------------------------------------------------------------
...
# ------------------------------
PANTILT_ON = (
    False  # Default= False Off, True= Enable Pan Tilt Hat Hardware (Load Drivers)
)
PANTILT_IS_PIMORONI = (
    True  # Default= True Use Pimoroni pantilehat, False= Use Waveshare pantilthat
)
PANTILT_HOME = (
    0,
    0,
...
PANO_NUM_RECYCLE = (
    True  # Default= True Recycle numbering when NUM MAX exceeded. False= Exit
)
...
VIDEO_SESSION_MIN = (
    30  # Default= 30 minutes  Run Recording Session then Exit  0=Continuous
)
...
VIDEO_NUM_RECYCLE_ON = (
    False  # Default= False when NumMax reached restart at NumStart instead of exiting
)
...
SPACE_TIMER_HOURS = (
    0  # Default= 0  0=Off or specify hours frequency to perform free disk space check
)
...
# -------------------
web_server_port = (
    8080  # Default= 8080 Web server access port eg http://192.168.1.100:8080
)
web_server_root = (
    "media"  # Default= "media" webserver root path to webserver image/video sub-folders
)
...
web_page_refresh_sec = (
    "900"  # Default= "900" seconds to wait for web page refresh  seconds (15 minutes)
)
...
web_max_list_entries = (
    0  # Default= 0 All or Specify Max right side file entries to show (must be > 1)
)
web_list_height = (
    web_image_height  # Right List - side menu height in px (link selection)
)
